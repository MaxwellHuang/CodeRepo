...
public class SnakeUtilTest {
    ...
    public void test1() throws Exception {
        ...
        List<Integer> teams = snakeUtil.createTeams(6);
        printLists(teams);
    }
    ...
    public void test2() throws Exception {
        SnakeUtil snakeUtil = new SnakeUtil();
        printSchedule(snakeUtil.createTeams(6));
    ...
    private void printLists(List<Integer> teams) {
        ...
        for (int i = 1; i < teams.size(); i++) {
            ...
            for (int j = 0; j < leftTeams.size(); j++) {
                ...
                System.out.println((String.format("%2s%1s%2s", (left == 0 ? "●" : left), "-", (right == 0 ? "●" : right))));
            }
    ...
    private void printSchedule(List<Integer> teams) {
        ...
        for (int i = 1; i < teams.size(); i++) {
            ...
            for (int j = 0; j < leftTeams.size(); j++) {
                ...
                Integer left = leftTeams.get(j);
                if (right != 0)
                    teamMap.get(right).add(left == 0 ? "●" : left.toString());
                if (left != 0)
                    teamMap.get(left).add(right == 0 ? "●" : right.toString());
            }
            snakeUtil.moveListElement(leftTeams, rightTeams);
        }
        System.out.println();
